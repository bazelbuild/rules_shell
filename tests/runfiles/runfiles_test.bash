...
function find_runfiles_lib() {
  ...
  if [[ ! -d "${RUNFILES_DIR:-/dev/null}" && ! -f "${RUNFILES_MANIFEST_FILE:-/dev/null}" ]]; then
    ...
    elif [[ -f "$0.runfiles/_main/shell/runfiles/runfiles.bash" ]]; then
      export RUNFILES_DIR="$0.runfiles"
    fi
  fi
  if [[ -f "${RUNFILES_DIR:-/dev/null}/_main/shell/runfiles/runfiles.bash" ]]; then
    echo "${RUNFILES_DIR}/_main/shell/runfiles/runfiles.bash"
  elif [[ -f "${RUNFILES_MANIFEST_FILE:-/dev/null}" ]]; then
    grep -m1 "^_main/shell/runfiles/runfiles.bash " \
